...
export class PartnershipsOverview extends connect(store)(
    ...
    return html`
            ...
            <div on-tap="_showPresetFilter" class="alert-row" no-collapse>
              <div slot="row-data">
                <div class="col-1 alerts-total">
                  [[presets.filteredPartnershipsOverview.length]]
                </div>
      ...
      <div class="paper-material" id="list" elevation="1">
        <data-table-header no-collapse>
          ...
          <data-table-column class="col-1"> Section </data-table-column>
          <div class="col-5 layout horizontal">
            ...
            <data-table-column
              class="col-3 text-right"
              field="outstanding_dct_amount_6_to_9_months_usd"
              sortable
            >
              Outstanding<br />
              DCT >6 Months
            ...
            <data-table-column
              class="col-3 text-right"
              field="outstanding_dct_amount_more_than_9_months_usd"
              sortable
            >
              Outstanding<br />
              DCT >9 Months
          ...
          <data-table-column
            class="col-1 text-right"
            field="alert_core_value_assessment"
            sortable
          >
            Flags
          </data-table-column>
          ...
          <data-table-row no-collapse>
            <div slot="row-data">
              <div class="col-3">
                ...
                <a
                  href="[[baseSite]]/pmp/partners/[[row.id]]/details"
                  app-name="pmp"
                  page="partners"
                  id="[[row.id]]/details"
                  class="cso-title"
              ...
              <div class="col-1 text-right">
                <a
                  class="blue-text"
                  href="[[baseSite]]/apd/action-points/list?partner=[[row.id]]&status=open"
                  >[[row.action_points]]</a
                >
              </div>
              <div class="col-1 text-right">
                <template
                  is="dom-if"
                  if="[[_checkCoreValuesAssessmentAlert(row.core_value_assessment_expiring)]]"
                >
                  <iron-icon
                    class="clock-icon [[_checkWarning(row.core_value_assessment_expiring)]]"
                    icon="device:access-time"
                  >
                  </iron-icon>
                  <paper-tooltip
                    >The Core Values Assessment will expire in
                    [[_getCoreValueAssessmentDays(row.core_value_assessment_expiring)]]
                    days.</paper-tooltip
                  >
                </template>
                <template
                  is="dom-if"
                  if="[[_checkCoreValuesAssessmentWarning(row.core_value_assessment_expiring)]]"
                >
                  <iron-icon
                    class="clock-icon warning"
                    icon="device:access-time"
                  >
                  </iron-icon>
                  <paper-tooltip
                    >The Core Values Assessment expired
                    [[_getCoreValueAssessmentDays(row.core_value_assessment_expiring,
                    'absolute')]] days ago.</paper-tooltip
                  >
                </template>
              </div>
  ...
  _init(active, sectors, requiredDataLoaded) {
    ...
    this.set(
      'selectedTypes',
      params.partner_types ? this._setSelectedTypes(params.partner_types) : []
    );
  ...
  rangeChanged() {
    ...
    if (this.currentFilter !== 'custom') {
      ...
      // @ts-ignore
      this.$.partnerListTemplate.items =
        this.filteredPartnershipsOverview.slice(
          // @ts-ignore
          this.visibleRange[0] - 1,
          // @ts-ignore
          this.visibleRange[1]
  ...
  _buildCsvDownloadUrl() {
    if (this.filteredPartnershipsOverview && this.active) {
      let pks = [];
      if (this.currentFilter === 'custom') {
        pks = this.filteredPartnershipsOverview.map((partner) =>
          prop('id', partner)
        );
      } else {
        // @ts-ignore
  ...
  _setSelectedTypes(typesStr) {
    ...
    if (!isEmpty(typesArr)) {
      return this.partnerTypes.filter(
        (s: any) => typesArr.indexOf(s.value) > -1
      );
    }
    return [];
  }
  ...
  _getCoreValueAssessmentDays(days, absolute?) {
    ...
    let expirationDate = dayjs(originalDate)
      .add(60, 'months')
      .subtract(1, 'days');
    if (absolute) {
  ...
  _showPresetFilter(event) {
    ...
    if (this.currentFilter === 'custom') {
      ...
      (
        this.$.partnershipsOverviewFooter as DataTableFooter
      ).filteredTotalResults = this.totalResults;
    } else {
      (this.$.customFilterPanel as HTMLElement).hidden = true;
      (
        this.$.partnershipsOverviewFooter as DataTableFooter
      ).filteredTotalResults = clickedFilter.total;
    }
  }
