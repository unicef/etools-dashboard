...
export class PartnershipOverviewData extends DataElementMixin(PolymerElement) {
  ...
  presetFilters = [
    {
      total: 0,
      id: '1',
      title: 'IPs with no recent Programmatic Visits (in the past 180 days)',
      status: '',
      filteredPartnershipsOverview: [],
    },
    {
      total: 0,
      id: '2',
      title: 'IPs with no Programmatic Visits',
      status: '',
      filteredPartnershipsOverview: [],
    },
    {
      total: 0,
      id: '3',
      title: 'IPs requiring new PCA in the coming CP',
      status: '',
      filteredPartnershipsOverview: [],
    },
    {
      total: 0,
      id: '4',
      title: 'IPs with active PD without a signed PCA',
      status: '',
      filteredPartnershipsOverview: [],
    },
  ];
  ...
  predicates = {
    searchString: (qs, partner) =>
      qs.length
        ? contains(qs.toLowerCase(), partner.name.toLowerCase()) ||
          contains(qs.toLowerCase(), partner.vendor_number)
        : true,
    sectors: (sectors, partner) =>
      sectors.length && partner.sections
        ? !!intersection(sectors, partner.sections.split('|')).length
        : true,
    name: (name, partner) => (name.length ? any(equals(partner), name) : true),
    outstanding: (outstanding, partner) =>
      outstanding
        ? !!parseInt(partner.outstanding_dct_amount_more_than_9_months_usd)
        : true,
    partnerType: (type, partner) =>
      type.length ? !!intersection(type, partner.partner_type).length : true,
  };
  ...
  createPresetFilters(results) {
    return [
      ...
      results
        .clone()
        .filter((partner) => partner.alert_active_pd_for_ended_pca),
    ];
  }
  ...
  query(params) {
    fireEvent(this, 'global-loading', {
      active: true,
      loadingSource: 'partnership-overview-data',
    });
    ...
    window.EtoolsDashboard.DexieDb.transaction(
      ...
      () => {
        let queryResult =
          window.EtoolsDashboard.DexieDb.partnershipsOverview.orderBy('name');
      ...
      .catch((err) =>
        console.error('Error querying partnerships-overview: ', err)
      )
      .finally(() => {
        fireEvent(this, 'global-loading', {
          loadingSource: 'partnership-overview-data',
