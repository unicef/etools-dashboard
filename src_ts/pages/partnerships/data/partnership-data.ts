...
export class PartnershipData extends DateMixin(
  DataElementMixin(PolymerElement)
) {
  ...
  presetFilters = [
    {
      total: 0,
      id: '1',
      title: 'PD/SSFA with passed start date and no FR was added',
      status: '(signed)',
      filteredPartnerships: [],
    },
    {
      total: 0,
      id: '2',
      title: 'PD/SSFA with planned amount not equal to FR amount',
    ...
    },
    {
      total: 0,
      id: '3',
      title: 'PD with no recent programmatic visit',
      status: '(active, ended)',
      filteredPartnerships: [],
    },
    {
      total: 0,
      id: '4',
      title: 'PD/SSFA expiring in the next 30 days',
    ...
    },
    {
      ...
      id: '5',
      title:
        'PD/SSFA with  actual disbursement amount less than FR planned amount',
      status: '(ended)',
      filteredPartnerships: [],
    },
    {
      total: 0,
      id: '6',
      title: 'PD/SSFA with missing Final Partnership Review',
      status: '(ended)',
      filteredPartnerships: [],
    },
  ];
  ...
  predicates = {
    searchString: (qs, partner) => {
      return qs.length && partner.partner_vendor_number
        ? contains(qs.toLowerCase(), partner.partner_name.toLowerCase()) ||
            contains(qs.toLowerCase(), partner.partner_vendor_number)
        : qs.length
        ? contains(qs.toLowerCase(), partner.partner_name.toLowerCase())
        : true;
    },
    status: (status, partner) =>
      status.length ? (contains(partner.status, status) ? true : false) : true,
    startAfterDate: (date, partner) =>
      isEmpty(partner.start)
        ? true
        : !isEmpty(date)
        ? this.prepareDate(partner.start) > this.prepareDate(date)
        : true,
    endBeforeDate: (date, partner) =>
      isEmpty(partner.end)
        ? true
        : !isEmpty(date)
        ? this.prepareDate(partner.end) < this.prepareDate(date)
        : true,
    startBeforeDate: (date, partner) =>
      isEmpty(partner.start)
        ? true
        : !isEmpty(date)
        ? this.prepareDate(partner.start) < this.prepareDate(date)
        : true,
    endAfterDate: (date, partner) =>
      isEmpty(partner.end)
        ? true
        : !isEmpty(date)
        ? this.prepareDate(partner.end) > this.prepareDate(date)
        : true,
    sectors: (sectors, partner) =>
      sectors.length
        ? !!intersection(sectors, [trim(partner.sections)]).length
        : true,
    offices: (offices, partner) =>
      offices.length
        ? !!intersection(offices, partner.offices_names.split(',')).length
        : true,
  };
  ...
  createPresetFilters(results) {
    return [
      ...
      results
        ...
        .filter(
          (partner) =>
            (partner.status === 'active' || partner.status === 'ended') &&
  ...
  query(params) {
    ...
    fireEvent(this, 'global-loading', {
      active: true,
      loadingSource: 'partnership-data',
    });
    window.EtoolsDashboard.DexieDb.transaction(
      ...
      () => {
        let queryResult =
          window.EtoolsDashboard.DexieDb.csoDashboard.orderBy('partner_name');
        if (order === 'desc') {
          queryResult = queryResult.reverse();
        }
        let queryResultCopy = queryResult.clone();
        this.set('presetResults', this.createPresetFilters(queryResultCopy));
        const callPred = (pred, partner) =>
          this.predicates[pred](params[pred], partner);
        queryResult = queryResult.filter((partner) =>
          predKeys.reduce((acc, pred) => acc && callPred(pred, partner), true)
        );
        return Dexie.Promise.all([
          queryResult.count(),
          queryResult.sortBy(sortBy || 'partner_name'),
          ...this.presetResults.map((res: any) =>
            res.sortBy(sortBy || 'partner_name')
          ),
        ]);
      }
    )
